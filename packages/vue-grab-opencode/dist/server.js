...
var binPath = path.resolve(__dirname$1, "../node_modules/.bin");
var envPath = process.env.PATH || process.env.Path || "";
var newPath = `${binPath}${path.delimiter}${envPath}`;
...
async function localCreateOpencodeServer(options = {}) {
  ...
  const opencodeBin = process.platform === "win32" ? path.join(binPath, "opencode.cmd") : path.join(binPath, "opencode");
